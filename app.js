/* ════════════════════════════════════════════════
   APP.JS — Trading Report · BAKERY GRP.68,78
   CP Axtra / Makro Thailand
   ════════════════════════════════════════════════
   โครงสร้างไฟล์:
   - index.html   โครงหน้าเว็บ (markup)
   - style.css    ดีไซน์ / ธีม
   - data.json    LOGO, รายการสินค้า (ITEMS_DATA), รายชื่อสาขา (STORES), บัญชี Admin
   - firebase.js  ตั้งค่า Firebase + ฟังก์ชันอ่าน/เขียนฐานข้อมูล
   - app.js       ไฟล์นี้ — Logic ทั้งหมดของแอป
   ════════════════════════════════════════════════ */

/* ════ DATA (โหลดจาก data.json ตอนเริ่มแอป) ════ */
let LOGO_URI = '';
let ITEMS_DATA = [];
let STORES = [];
let ADMIN = { u: 'admin', p: '', name: 'ผู้ดูแลระบบ' };
...
let ALL_CLS=['ALL'];
function buildAllCls(){ALL_CLS=['ALL',...new Set(ITEMS_DATA.map(i=>i.class).filter(Boolean))].sort((a,b)=>{if(a==='ALL')return-1;if(b==='ALL')return 1;const na=Number(a),nb=Number(b);if(!isNaN(na)&&!isNaN(nb))return na-nb;return a.localeCompare(b);});}
...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    console.error(e);
    document.body.innerHTML = `<div style="padding:40px;text-align:center;font-family:sans-serif;color:#E03244">
      <h2>⚠️ โหลดข้อมูลแอปไม่สำเร็จ</h2>
      <p>กรุณาตรวจสอบว่าไฟล์ <code>data.json</code> อยู่ในโฟลเดอร์เดียวกับ index.html</p>
      <p style="color:#6B7A90;font-size:13px">${e.message}</p>
    </div>`;
    return;
  }
